...
for (let i = 0; i < items.length; i++) {
  ...
  valid.push({
    json: {
      ...
      asin: p.asin ? String(p.asin).trim() : null,
      rating: p.rating && typeof p.rating === 'object'
        ? { value: Number(p.rating.value) || null, count: Number(p.rating.count) || null }
        : null,
...
const userPrompt = [
  ...
  '',
  '{',
  '  "category": "<one of EXACTLY: summer-gadgets, beach-essentials, tiktok-finds, aesthetic-room, travel, skincare-summer, pet-summer>",',
  '  "tags": ["<zero or more of EXACTLY: trending, tiktok, under-25, viral>"],',
  ...
  '',
  'Rules:',
  '- "category" MUST be one of the 7 slugs above, nothing else.',
  '- "tags" MUST only contain values from the 4 allowed tags above (or be empty).',
  '- Do NOT invent new category or tag values.',
  '',
  'Product:',
  '- SKU: ' + product.sku,
  '- Title: ' + product.title,
...
//    Coerce category + tags to the canonical enums in lib/types.ts so the
//    site never receives an out-of-schema value.
const CATEGORY_SLUGS = ['summer-gadgets', 'beach-essentials', 'tiktok-finds', 'aesthetic-room', 'travel', 'skincare-summer', 'pet-summer'];
const PRODUCT_TAGS = ['trending', 'tiktok', 'under-25', 'viral'];

const rawCategory = String(parsed.category || '').trim().toLowerCase();
const category = CATEGORY_SLUGS.includes(rawCategory) ? rawCategory : 'summer-gadgets';

const tags = Array.isArray(parsed.tags)
  ? Array.from(new Set(parsed.tags.map(t => String(t).trim().toLowerCase()).filter(t => PRODUCT_TAGS.includes(t))))
  : [];
...
// Build a stable, URL-safe id the same way scripts/import-products.mjs does
// (brand-title), so /finds/[slug] ids stay consistent across both pipelines.
...
const brandSlug = slugify(product.brand) || 'find';
const slug = (brandSlug + '-' + slugify(product.title)).slice(0, 80);
...
const website_payload = {
  ...
  brand: product.brand || 'See Amazon listing',
  price: product.price,
  rating: product.rating && product.rating.value != null
    ? { value: product.rating.value, count: product.rating.count }
    : null,
  category: ai.category,
  tags: ai.tags,
  image: product.image_url,
  imageSource: product.image_url ? 'amazon' : 'curated',
  asin: product.asin || null,
  affiliateUrl: product.affiliate_url,
  source: 'wf-01:affiliate-product-pipeline',
...
const canva_payload = {
  ...
  fields: {
    ...
    subheadline: ai.viral_hooks[1] || ai.category,
